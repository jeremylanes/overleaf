...
${frame}`),highlighted?colors2.reset(frame):frame}function _default2(rawLines,lineNumber,colNumber,opts={}){if(!deprecationWarningShown){deprecationWarningShown=!0;let message="Passing lineNumber and colNumber is deprecated to @babel/code-frame. Please use `codeFrameColumns`.";if(process.emitWarning)process.emitWarning(message,"DeprecationWarning");else {let deprecationError=new Error(message);deprecationError.name="DeprecationWarning",console.warn(new Error(message));}}return colNumber=Math.max(colNumber,0),codeFrameColumns3(rawLines,{start:{column:colNumber,line:lineNumber}},opts)}}}),require_ignore=__commonJS({"node_modules/ignore/index.js"(exports,module){function makeArray(subject){return Array.isArray(subject)?subject:[subject]}var EMPTY="",SPACE=" ",ESCAPE="\\",REGEX_TEST_BLANK_LINE=/^\s+$/,REGEX_INVALID_TRAILING_BACKSLASH=/(?:[^\\]|^)\\$/,REGEX_REPLACE_LEADING_EXCAPED_EXCLAMATION=/^\\!/,REGEX_REPLACE_LEADING_EXCAPED_HASH=/^\\#/,REGEX_SPLITALL_CRLF=/\r?\n/g,REGEX_TEST_INVALID_PATH=/^\.*\/|^\.+$/,SLASH="/",TMP_KEY_IGNORE="node-ignore";typeof Symbol<"u"&&(TMP_KEY_IGNORE=Symbol.for("node-ignore"));var KEY_IGNORE=TMP_KEY_IGNORE,define=(object,key2,value)=>Object.defineProperty(object,key2,{value}),REGEX_REGEXP_RANGE=/([0-z])-([0-z])/g,RETURN_FALSE=()=>!1,sanitizeRange=range=>range.replace(REGEX_REGEXP_RANGE,(match,from,to)=>from.charCodeAt(0)<=to.charCodeAt(0)?match:EMPTY),cleanRangeBackSlash=slashes=>{let{length}=slashes;return slashes.slice(0,length-length%2)},REPLACERS=[[/^\uFEFF/,()=>EMPTY],[/\\?\s+$/,match=>match.indexOf("\\")===0?SPACE:EMPTY],[/\\\s/g,()=>SPACE],[/[\\$.|*+(){^]/g,match=>`\\${match}`],[/(?!\\)\?/g,()=>"[^/]"],[/^\//,()=>"^"],[/\//g,()=>"\\/"],[/^\^*\\\*\\\*\\\//,()=>"^(?:.*\\/)?"],[/^(?=[^^])/,function(){return /\/(?!$)/.test(this)?"^":"(?:^|\\/)"}],[/\\\/\\\*\\\*(?=\\\/|$)/g,(_,index,str2)=>index+6<str2.length?"(?:\\/[^\\/]+)*":"\\/.+"],[/(^|[^\\]+)(\\\*)+(?=.+)/g,(_,p1,p2)=>{let unescaped=p2.replace(/\\\*/g,"[^\\/]*");return p1+unescaped}],[/\\\\\\(?=[$.|*+(){^])/g,()=>ESCAPE],[/\\\\/g,()=>ESCAPE],[/(\\)?\[([^\]/]*?)(\\*)($|\])/g,(match,leadEscape,range,endEscape,close)=>leadEscape===ESCAPE?`\\[${range}${cleanRangeBackSlash(endEscape)}${close}`:close==="]"&&endEscape.length%2===0?`[${sanitizeRange(range)}${endEscape}]`:"[]"],[/(?:[^*])$/,match=>/\/$/.test(match)?`${match}$`:`${match}(?=$|\\/$)`],[/(\^|\\\/)?\\\*$/,(_,p1)=>`${p1?`${p1}[^/]+`:"[^/]*"}(?=$|\\/$)`]],regexCache=Object.create(null),makeRegex=(pattern,ignoreCase)=>{let source2=regexCache[pattern];return source2||(source2=REPLACERS.reduce((prev,current)=>prev.replace(current[0],current[1].bind(pattern)),pattern),regexCache[pattern]=source2),ignoreCase?new RegExp(source2,"i"):new RegExp(source2)},isString=subject=>typeof subject=="string",checkPattern=pattern=>pattern&&isString(pattern)&&!REGEX_TEST_BLANK_LINE.test(pattern)&&!REGEX_INVALID_TRAILING_BACKSLASH.test(pattern)&&pattern.indexOf("#")!==0,splitPattern=pattern=>pattern.split(REGEX_SPLITALL_CRLF),IgnoreRule=class{constructor(origin,pattern,negative,regex){this.origin=origin,this.pattern=pattern,this.negative=negative,this.regex=regex;}},createRule=(pattern,ignoreCase)=>{let origin=pattern,negative=!1;pattern.indexOf("!")===0&&(negative=!0,pattern=pattern.substr(1)),pattern=pattern.replace(REGEX_REPLACE_LEADING_EXCAPED_EXCLAMATION,"!").replace(REGEX_REPLACE_LEADING_EXCAPED_HASH,"#");let regex=makeRegex(pattern,ignoreCase);return new IgnoreRule(origin,pattern,negative,regex)},throwError2=(message,Ctor)=>{throw new Ctor(message)},checkPath=(path13,originalPath,doThrow)=>isString(path13)?path13?checkPath.isNotRelative(path13)?doThrow(`path should be a \`path.relative()\`d string, but got "${originalPath}"`,RangeError):!0:doThrow("path must not be empty",TypeError):doThrow(`path must be a string, but got \`${originalPath}\``,TypeError),isNotRelative=path13=>REGEX_TEST_INVALID_PATH.test(path13);checkPath.isNotRelative=isNotRelative,checkPath.convert=p=>p;var Ignore=class{constructor({ignorecase=!0,ignoreCase=ignorecase,allowRelativePaths=!1}={}){define(this,KEY_IGNORE,!0),this._rules=[],this._ignoreCase=ignoreCase,this._allowRelativePaths=allowRelativePaths,this._initCache();}_initCache(){this._ignoreCache=Object.create(null),this._testCache=Object.create(null);}_addPattern(pattern){if(pattern&&pattern[KEY_IGNORE]){this._rules=this._rules.concat(pattern._rules),this._added=!0;return}if(checkPattern(pattern)){let rule=createRule(pattern,this._ignoreCase);this._added=!0,this._rules.push(rule);}}add(pattern){return this._added=!1,makeArray(isString(pattern)?splitPattern(pattern):pattern).forEach(this._addPattern,this),this._added&&this._initCache(),this}addPattern(pattern){return this.add(pattern)}_testOne(path13,checkUnignored){let ignored=!1,unignored=!1;return this._rules.forEach(rule=>{let{negative}=rule;if(unignored===negative&&ignored!==unignored||negative&&!ignored&&!unignored&&!checkUnignored)return;rule.regex.test(path13)&&(ignored=!negative,unignored=negative);}),{ignored,unignored}}_test(originalPath,cache3,checkUnignored,slices){let path13=originalPath&&checkPath.convert(originalPath);return checkPath(path13,originalPath,this._allowRelativePaths?RETURN_FALSE:throwError2),this._t(path13,cache3,checkUnignored,slices)}_t(path13,cache3,checkUnignored,slices){if(path13 in cache3)return cache3[path13];if(slices||(slices=path13.split(SLASH)),slices.pop(),!slices.length)return cache3[path13]=this._testOne(path13,checkUnignored);let parent=this._t(slices.join(SLASH)+SLASH,cache3,checkUnignored,slices);return cache3[path13]=parent.ignored?parent:this._testOne(path13,checkUnignored)}ignores(path13){return this._test(path13,this._ignoreCache,!1).ignored}createFilter(){return path13=>!this.ignores(path13)}filter(paths){return makeArray(paths).filter(this.createFilter())}test(path13){return this._test(path13,this._testCache,!0)}},factory=options8=>new Ignore(options8),isPathValid=path13=>checkPath(path13&&checkPath.convert(path13),path13,RETURN_FALSE);if(factory.isPathValid=isPathValid,factory.default=factory,module.exports=factory,typeof process<"u"&&(process.env&&process.env.IGNORE_TEST_WIN32||process.platform==="win32")){let makePosix=str2=>/^\\\\\?\\/.test(str2)||/["<>|\u0000-\u001F]+/u.test(str2)?str2:str2.replace(/\\/g,"/");checkPath.convert=makePosix;let REGIX_IS_WINDOWS_PATH_ABSOLUTE=/^[a-z]:\//i;checkPath.isNotRelative=path13=>REGIX_IS_WINDOWS_PATH_ABSOLUTE.test(path13)||isNotRelative(path13);}}}),require_readlines=__commonJS({"node_modules/n-readlines/readlines.js"(exports,module){var fs7=__require("fs"),LineByLine=class{constructor(file,options8){options8=options8||{},options8.readChunk||(options8.readChunk=1024),options8.newLineCharacter?options8.newLineCharacter=options8.newLineCharacter.charCodeAt(0):options8.newLineCharacter=10,typeof file=="number"?this.fd=file:this.fd=fs7.openSync(file,"r"),this.options=options8,this.newLineCharacter=options8.newLineCharacter,this.reset();}_searchInBuffer(buffer2,hexNeedle){let found=-1;for(let i2=0;i2<=buffer2.length;i2++)if(buffer2[i2]===hexNeedle){found=i2;break}return found}reset(){this.eofReached=!1,this.linesCache=[],this.fdPosition=0;}close(){fs7.closeSync(this.fd),this.fd=null;}_extractLines(buffer2){let line3,lines=[],bufferPosition=0,lastNewLineBufferPosition=0;for(;;){let bufferPositionValue=buffer2[bufferPosition++];if(bufferPositionValue===this.newLineCharacter)line3=buffer2.slice(lastNewLineBufferPosition,bufferPosition),lines.push(line3),lastNewLineBufferPosition=bufferPosition;else if(bufferPositionValue===void 0)break}let leftovers=buffer2.slice(lastNewLineBufferPosition,bufferPosition);return leftovers.length&&lines.push(leftovers),lines}_readChunk(lineLeftovers){let totalBytesRead=0,bytesRead,buffers=[];do{let readBuffer=Buffer.alloc(this.options.readChunk);bytesRead=fs7.readSync(this.fd,readBuffer,0,this.options.readChunk,this.fdPosition),totalBytesRead=totalBytesRead+bytesRead,this.fdPosition=this.fdPosition+bytesRead,buffers.push(readBuffer);}while(bytesRead&&this._searchInBuffer(buffers[buffers.length-1],this.options.newLineCharacter)===-1);let bufferData=Buffer.concat(buffers);return bytesRead<this.options.readChunk&&(this.eofReached=!0,bufferData=bufferData.slice(0,totalBytesRead)),totalBytesRead&&(this.linesCache=this._extractLines(bufferData),lineLeftovers&&(this.linesCache[0]=Buffer.concat([lineLeftovers,this.linesCache[0]]))),totalBytesRead}next(){if(!this.fd)return !1;let line3=!1;if(this.eofReached&&this.linesCache.length===0)return line3;let bytesRead;return this.linesCache.length||(bytesRead=this._readChunk()),this.linesCache.length&&(line3=this.linesCache.shift(),line3[line3.length-1]!==this.newLineCharacter&&(bytesRead=this._readChunk(line3),bytesRead&&(line3=this.linesCache.shift()))),this.eofReached&&this.linesCache.length===0&&this.close(),line3&&line3[line3.length-1]===this.newLineCharacter&&(line3=line3.slice(0,line3.length-1)),line3}};module.exports=LineByLine;}}),src_exports={};__export3(src_exports,{__debug:()=>debugApis,__internal:()=>sharedWithCli,check:()=>check,clearConfigCache:()=>clearCache3,doc:()=>doc_exports,format:()=>format2,formatWithCursor:()=>formatWithCursor2,getFileInfo:()=>getFileInfo2,getSupportInfo:()=>getSupportInfo2,resolveConfig:()=>resolveConfig,resolveConfigFile:()=>resolveConfigFile,util:()=>public_exports2,version:()=>version_evaluate_default});function Diff(){}Diff.prototype={diff:function(oldString,newString){var _options$timeout,options8=arguments.length>2&&arguments[2]!==void 0?arguments[2]:{},callback=options8.callback;typeof options8=="function"&&(callback=options8,options8={}),this.options=options8;var self=this;function done(value){return callback?(setTimeout(function(){callback(void 0,value);},0),!0):value}oldString=this.castInput(oldString),newString=this.castInput(newString),oldString=this.removeEmpty(this.tokenize(oldString)),newString=this.removeEmpty(this.tokenize(newString));var newLen=newString.length,oldLen=oldString.length,editLength=1,maxEditLength=newLen+oldLen;options8.maxEditLength&&(maxEditLength=Math.min(maxEditLength,options8.maxEditLength));var maxExecutionTime=(_options$timeout=options8.timeout)!==null&&_options$timeout!==void 0?_options$timeout:1/0,abortAfterTimestamp=Date.now()+maxExecutionTime,bestPath=[{oldPos:-1,lastComponent:void 0}],newPos=this.extractCommon(bestPath[0],newString,oldString,0);if(bestPath[0].oldPos+1>=oldLen&&newPos+1>=newLen)return done([{value:this.join(newString),count:newString.length}]);var minDiagonalToConsider=-1/0,maxDiagonalToConsider=1/0;function execEditLength(){for(var diagonalPath=Math.max(minDiagonalToConsider,-editLength);diagonalPath<=Math.min(maxDiagonalToConsider,editLength);diagonalPath+=2){var basePath=void 0,removePath=bestPath[diagonalPath-1],addPath=bestPath[diagonalPath+1];removePath&&(bestPath[diagonalPath-1]=void 0);var canAdd=!1;if(addPath){var addPathNewPos=addPath.oldPos-diagonalPath;canAdd=addPath&&0<=addPathNewPos&&addPathNewPos<newLen;}var canRemove=removePath&&removePath.oldPos+1<oldLen;if(!canAdd&&!canRemove){bestPath[diagonalPath]=void 0;continue}if(!canRemove||canAdd&&removePath.oldPos+1<addPath.oldPos?basePath=self.addToPath(addPath,!0,void 0,0):basePath=self.addToPath(removePath,void 0,!0,1),newPos=self.extractCommon(basePath,newString,oldString,diagonalPath),basePath.oldPos+1>=oldLen&&newPos+1>=newLen)return done(buildValues(self,basePath.lastComponent,newString,oldString,self.useLongestToken));bestPath[diagonalPath]=basePath,basePath.oldPos+1>=oldLen&&(maxDiagonalToConsider=Math.min(maxDiagonalToConsider,diagonalPath-1)),newPos+1>=newLen&&(minDiagonalToConsider=Math.max(minDiagonalToConsider,diagonalPath+1));}editLength++;}if(callback)(function exec(){setTimeout(function(){if(editLength>maxEditLength||Date.now()>abortAfterTimestamp)return callback();execEditLength()||exec();},0);})();else for(;editLength<=maxEditLength&&Date.now()<=abortAfterTimestamp;){var ret=execEditLength();if(ret)return ret}},addToPath:function(path13,added,removed,oldPosInc){var last=path13.lastComponent;return last&&last.added===added&&last.removed===removed?{oldPos:path13.oldPos+oldPosInc,lastComponent:{count:last.count+1,added,removed,previousComponent:last.previousComponent}}:{oldPos:path13.oldPos+oldPosInc,lastComponent:{count:1,added,removed,previousComponent:last}}},extractCommon:function(basePath,newString,oldString,diagonalPath){for(var newLen=newString.length,oldLen=oldString.length,oldPos=basePath.oldPos,newPos=oldPos-diagonalPath,commonCount=0;newPos+1<newLen&&oldPos+1<oldLen&&this.equals(newString[newPos+1],oldString[oldPos+1]);)newPos++,oldPos++,commonCount++;return commonCount&&(basePath.lastComponent={count:commonCount,previousComponent:basePath.lastComponent}),basePath.oldPos=oldPos,newPos},equals:function(left,right){return this.options.comparator?this.options.comparator(left,right):left===right||this.options.ignoreCase&&left.toLowerCase()===right.toLowerCase()},removeEmpty:function(array2){for(var ret=[],i2=0;i2<array2.length;i2++)array2[i2]&&ret.push(array2[i2]);return ret},castInput:function(value){return value},tokenize:function(value){return value.split("")},join:function(chars){return chars.join("")}};function buildValues(diff2,lastComponent,newString,oldString,useLongestToken){for(var components=[],nextComponent;lastComponent;)components.push(lastComponent),nextComponent=lastComponent.previousComponent,delete lastComponent.previousComponent,lastComponent=nextComponent;components.reverse();for(var componentPos=0,componentLen=components.length,newPos=0,oldPos=0;componentPos<componentLen;componentPos++){var component=components[componentPos];if(component.removed){if(component.value=diff2.join(oldString.slice(oldPos,oldPos+component.count)),oldPos+=component.count,componentPos&&components[componentPos-1].added){var tmp=components[componentPos-1];components[componentPos-1]=components[componentPos],components[componentPos]=tmp;}}else {if(!component.added&&useLongestToken){var value=newString.slice(newPos,newPos+component.count);value=value.map(function(value2,i2){var oldValue=oldString[oldPos+i2];return oldValue.length>value2.length?oldValue:value2}),component.value=diff2.join(value);}else component.value=diff2.join(newString.slice(newPos,newPos+component.count));newPos+=component.count,component.added||(oldPos+=component.count);}}var finalComponent=components[componentLen-1];return componentLen>1&&typeof finalComponent.value=="string"&&(finalComponent.added||finalComponent.removed)&&diff2.equals("",finalComponent.value)&&(components[componentLen-2].value+=finalComponent.value,components.pop()),components}var extendedWordChars=/^[A-Za-z\xC0-\u02C6\u02C8-\u02D7\u02DE-\u02FF\u1E00-\u1EFF]+$/,reWhitespace=/\S/,wordDiff=new Diff;wordDiff.equals=function(left,right){return this.options.ignoreCase&&(left=left.toLowerCase(),right=right.toLowerCase()),left===right||this.options.ignoreWhitespace&&!reWhitespace.test(left)&&!reWhitespace.test(right)};wordDiff.tokenize=function(value){for(var tokens=value.split(/([^\S\r\n]+|[()[\]{}'"\r\n]|\b)/),i2=0;i2<tokens.length-1;i2++)!tokens[i2+1]&&tokens[i2+2]&&extendedWordChars.test(tokens[i2])&&extendedWordChars.test(tokens[i2+2])&&(tokens[i2]+=tokens[i2+2],tokens.splice(i2+1,2),i2--);return tokens};var lineDiff=new Diff;lineDiff.tokenize=function(value){this.options.stripTrailingCr&&(value=value.replace(/\r\n/g,`
`));var retLines=[],linesAndNewlines=value.split(/(\n|\r\n)/);linesAndNewlines[linesAndNewlines.length-1]||linesAndNewlines.pop();for(var i2=0;i2<linesAndNewlines.length;i2++){var line3=linesAndNewlines[i2];i2%2&&!this.options.newlineIsToken?retLines[retLines.length-1]+=line3:(this.options.ignoreWhitespace&&(line3=line3.trim()),retLines.push(line3));}return retLines};function diffLines(oldStr,newStr,callback){return lineDiff.diff(oldStr,newStr,callback)}var sentenceDiff=new Diff;sentenceDiff.tokenize=function(value){return value.split(/(\S.+?[.!?])(?=\s+|$)/)};var cssDiff=new Diff;cssDiff.tokenize=function(value){return value.split(/([{}:;,]|\s+)/)};function _typeof(obj){"@babel/helpers - typeof";return typeof Symbol=="function"&&typeof Symbol.iterator=="symbol"?_typeof=function(obj2){return typeof obj2}:_typeof=function(obj2){return obj2&&typeof Symbol=="function"&&obj2.constructor===Symbol&&obj2!==Symbol.prototype?"symbol":typeof obj2},_typeof(obj)}function _toConsumableArray(arr){return _arrayWithoutHoles(arr)||_iterableToArray(arr)||_unsupportedIterableToArray(arr)||_nonIterableSpread()}function _arrayWithoutHoles(arr){if(Array.isArray(arr))return _arrayLikeToArray(arr)}function _iterableToArray(iter){if(typeof Symbol<"u"&&Symbol.iterator in Object(iter))return Array.from(iter)}function _unsupportedIterableToArray(o,minLen){if(o){if(typeof o=="string")return _arrayLikeToArray(o,minLen);var n=Object.prototype.toString.call(o).slice(8,-1);if(n==="Object"&&o.constructor&&(n=o.constructor.name),n==="Map"||n==="Set")return Array.from(o);if(n==="Arguments"||/^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n))return _arrayLikeToArray(o,minLen)}}function _arrayLikeToArray(arr,len){(len==null||len>arr.length)&&(len=arr.length);for(var i2=0,arr2=new Array(len);i2<len;i2++)arr2[i2]=arr[i2];return arr2}function _nonIterableSpread(){throw new TypeError(`Invalid attempt to spread non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`)}var objectPrototypeToString=Object.prototype.toString,jsonDiff=new Diff;jsonDiff.useLongestToken=!0;jsonDiff.tokenize=lineDiff.tokenize;jsonDiff.castInput=function(value){var _this$options=this.options,undefinedReplacement=_this$options.undefinedReplacement,_this$options$stringi=_this$options.stringifyReplacer,stringifyReplacer=_this$options$stringi===void 0?function(k,v){return typeof v>"u"?undefinedReplacement:v}:_this$options$stringi;return typeof value=="string"?value:JSON.stringify(canonicalize(value,null,null,stringifyReplacer),stringifyReplacer,"  ")};jsonDiff.equals=function(left,right){return Diff.prototype.equals.call(jsonDiff,left.replace(/,([\r\n])/g,"$1"),right.replace(/,([\r\n])/g,"$1"))};function canonicalize(obj,stack2,replacementStack,replacer,key2){stack2=stack2||[],replacementStack=replacementStack||[],replacer&&(obj=replacer(key2,obj));var i2;for(i2=0;i2<stack2.length;i2+=1)if(stack2[i2]===obj)return replacementStack[i2];var canonicalizedObj;if(objectPrototypeToString.call(obj)==="[object Array]"){for(stack2.push(obj),canonicalizedObj=new Array(obj.length),replacementStack.push(canonicalizedObj),i2=0;i2<obj.length;i2+=1)canonicalizedObj[i2]=canonicalize(obj[i2],stack2,replacementStack,replacer,key2);return stack2.pop(),replacementStack.pop(),canonicalizedObj}if(obj&&obj.toJSON&&(obj=obj.toJSON()),_typeof(obj)==="object"&&obj!==null){stack2.push(obj),canonicalizedObj={},replacementStack.push(canonicalizedObj);var sortedKeys=[],_key;for(_key in obj)obj.hasOwnProperty(_key)&&sortedKeys.push(_key);for(sortedKeys.sort(),i2=0;i2<sortedKeys.length;i2+=1)_key=sortedKeys[i2],canonicalizedObj[_key]=canonicalize(obj[_key],stack2,replacementStack,replacer,_key);stack2.pop(),replacementStack.pop();}else canonicalizedObj=obj;return canonicalizedObj}var arrayDiff=new Diff;arrayDiff.tokenize=function(value){return value.slice()};arrayDiff.join=arrayDiff.removeEmpty=function(value){return value};function diffArrays(oldArr,newArr,callback){return arrayDiff.diff(oldArr,newArr,callback)}function structuredPatch(oldFileName,newFileName,oldStr,newStr,oldHeader,newHeader,options8){options8||(options8={}),typeof options8.context>"u"&&(options8.context=4);var diff2=diffLines(oldStr,newStr,options8);if(!diff2)return;diff2.push({value:"",lines:[]});function contextLines(lines){return lines.map(function(entry){return " "+entry})}for(var hunks=[],oldRangeStart=0,newRangeStart=0,curRange=[],oldLine=1,newLine=1,_loop=function(i22){var current=diff2[i22],lines=current.lines||current.value.replace(/\n$/,"").split(`
`);if(current.lines=lines,current.added||current.removed){var _curRange;if(!oldRangeStart){var prev=diff2[i22-1];oldRangeStart=oldLine,newRangeStart=newLine,prev&&(curRange=options8.context>0?contextLines(prev.lines.slice(-options8.context)):[],oldRangeStart-=curRange.length,newRangeStart-=curRange.length);}(_curRange=curRange).push.apply(_curRange,_toConsumableArray(lines.map(function(entry){return (current.added?"+":"-")+entry}))),current.added?newLine+=lines.length:oldLine+=lines.length;}else {if(oldRangeStart)if(lines.length<=options8.context*2&&i22<diff2.length-2){var _curRange2;(_curRange2=curRange).push.apply(_curRange2,_toConsumableArray(contextLines(lines)));}else {var _curRange3,contextSize=Math.min(lines.length,options8.context);(_curRange3=curRange).push.apply(_curRange3,_toConsumableArray(contextLines(lines.slice(0,contextSize))));var hunk={oldStart:oldRangeStart,oldLines:oldLine-oldRangeStart+contextSize,newStart:newRangeStart,newLines:newLine-newRangeStart+contextSize,lines:curRange};if(i22>=diff2.length-2&&lines.length<=options8.context){var oldEOFNewline=/\n$/.test(oldStr),newEOFNewline=/\n$/.test(newStr),noNlBeforeAdds=lines.length==0&&curRange.length>hunk.oldLines;!oldEOFNewline&&noNlBeforeAdds&&oldStr.length>0&&curRange.splice(hunk.oldLines,0,"\\ No newline at end of file"),(!oldEOFNewline&&!noNlBeforeAdds||!newEOFNewline)&&curRange.push("\\ No newline at end of file");}hunks.push(hunk),oldRangeStart=0,newRangeStart=0,curRange=[];}oldLine+=lines.length,newLine+=lines.length;}},i2=0;i2<diff2.length;i2++)_loop(i2);return {oldFileName,newFileName,oldHeader,newHeader,hunks}}function formatPatch(diff2){if(Array.isArray(diff2))return diff2.map(formatPatch).join(`
...
`)+`
`}function createTwoFilesPatch(oldFileName,newFileName,oldStr,newStr,oldHeader,newHeader,options8){return formatPatch(structuredPatch(oldFileName,newFileName,oldStr,newStr,oldHeader,newHeader,options8))}var import_fast_glob=__toESM(require_out4(),1),apiDescriptor={key:key2=>/^[$_a-zA-Z][$_a-zA-Z0-9]*$/.test(key2)?key2:JSON.stringify(key2),value(value){if(value===null||typeof value!="object")return JSON.stringify(value);if(Array.isArray(value))return `[${value.map(subValue=>apiDescriptor.value(subValue)).join(", ")}]`;let keys=Object.keys(value);return keys.length===0?"{}":`{ ${keys.map(key2=>`${apiDescriptor.key(key2)}: ${apiDescriptor.value(value[key2])}`).join(", ")} }`},pair:({key:key2,value})=>apiDescriptor.value({[key2]:value})};init_source();var commonDeprecatedHandler=(keyOrPair,redirectTo,{descriptor})=>{let messages2=[`${source_default.yellow(typeof keyOrPair=="string"?descriptor.key(keyOrPair):descriptor.pair(keyOrPair))} is deprecated`];return redirectTo&&messages2.push(`we now treat it as ${source_default.blue(typeof redirectTo=="string"?descriptor.key(redirectTo):descriptor.pair(redirectTo))}`),messages2.join("; ")+"."};init_source();var VALUE_NOT_EXIST=Symbol.for("vnopts.VALUE_NOT_EXIST"),VALUE_UNCHANGED=Symbol.for("vnopts.VALUE_UNCHANGED"),INDENTATION=" ".repeat(2),commonInvalidHandler=(key2,value,utils2)=>{let{text,list}=utils2.normalizeExpectedResult(utils2.schemas[key2].expected(utils2)),descriptions=[];return text&&descriptions.push(getDescription(key2,value,text,utils2.descriptor)),list&&descriptions.push([getDescription(key2,value,list.title,utils2.descriptor)].concat(list.values.map(valueDescription=>getListDescription(valueDescription,utils2.loggerPrintWidth))).join(`
...
${error.message}`,error}}var loaders={async".toml"(file){let content=await read_file_default(file);try{return await(0,import_parse_async.default)(content)}catch(error){throw error.message=`TOML Error in ${file}:
